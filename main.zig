const std = @import("std");// ============================================================================// 1. PALETA DE CORES E TEMA VISUAL TUI (ANSI)// ============================================================================pub const Theme = struct {pub const red = "\x1b[38;2;248;113;113m";pub const green = "\x1b[38;2;74;222;128m";pub const yellow = "\x1b[38;2;250;204;21m";pub const blue = "\x1b[38;2;96;165;250m";pub const purple = "\x1b[38;2;192;132;252m";pub const cyan = "\x1b[38;2;56;189;248m";pub const orange = "\x1b[38;2;251;146;60m";pub const bold = "\x1b[1m";pub const dim = "\x1b[2m";pub const reset = "\x1b[0m";pub const clear_screen = "\x1b[2J\x1b[H";};// ============================================================================// 2. MODELOS DE DADOS E CONTRATOS DO PIPELINE DE AGENTES// ============================================================================pub const EventoTransacional = struct {id: u64,payload: []const u8,valor_eur: f64,status: []const u8,};pub const ContextoRuntime = struct {allocator: std.mem.Allocator,executando: bool = true,mutex: std.Thread.Mutex = .{},pub fn init(allocator: std.mem.Allocator) ContextoRuntime {
...
pub const AgenteHandlerFn = *const fn (ctx: *ContextoRuntime, ev: *EventoTransacional) bool;pub const RegistoAgente = struct {nome: []const u8,handler: AgenteHandlerFn,};// ============================================================================// 3. PARSER 2FLOW EM TEMPO DE COMPILAÇÃO (COMPTIME AST)// ============================================================================pub const TipoNoFlow = enum {modulo,sequencia,paralelo,};pub const NoFlowAST = struct {tipo: TipoNoFlow,nome: []const u8 = "",filhos: []const NoFlowAST = &.{},compensador_erro: []const u8 = "", // Suporte a !-> Rollback};const TokenKind = enum { identifier, arrow, error_arrow, open_bracket, close_bracket, comma };const Token = struct { kind: TokenKind, text: []const u8 };fn tokenize(comptime input: []const u8) []const Token {comptime {var tokens: []const Token = &.{};var i: usize = 0;    while (i < input.len) {
        const c = input[i];
...
}pub fn parse2Flow(comptime script: []const u8) NoFlowAST {comptime {const tokens = tokenize(script);const res = parseExpression(tokens, 0);return res.node;}}// ============================================================================// 4. MOTOR DE EXECUÇÃO E ORQUESTRAÇÃO DE DADOS (RUNTIME ORCHESTRATOR)// ============================================================================pub const TwoFlowOrchestrator = struct {ctx: *ContextoRuntime,catalogo: std.StringHashMap(AgenteHandlerFn),pub fn init(ctx: *ContextoRuntime) TwoFlowOrchestrator {
    return .{
        .ctx = ctx,
...
pub fn executarFlow(self: *TwoFlowOrchestrator, comptime node: NoFlowAST, ev: *EventoTransacional) bool {
    ...
    switch (node.tipo) {
        ...
        .paralelo => {
            ...
            // Execução paralela dos ramos em threads independentes
            const WorkerCtx = struct {
                orch: *TwoFlowOrchestrator,
                sub_node: NoFlowAST,
                evento: *EventoTransacional,
                resultado: bool,
            };

            var resultados = [_]bool{true} ** 8;
            var threads = std.ArrayList(std.Thread).init(self.ctx.allocator);
            defer threads.deinit();

            inline for (node.filhos, 0..) |filho, i| {
                // Cópia local de ponteiro para thread segura
                const t = std.Thread.spawn(.{}, struct {
                    fn run(orch: *TwoFlowOrchestrator, child: NoFlowAST, event: *EventoTransacional, res: *bool) void {
                        res.* = orch.executarFlow(child, event);
                    }
                }.run, .{ self, filho, ev, &resultados[i] }) catch {
...
fn agenteValidarOrdem(ctx: *ContextoRuntime, ev: *EventoTransacional) bool {
  ...
  std.time.sleep(40 * std.time.ns_per_ms);ev.status = "VALIDADO";return true;
}
  ...
  std.time.sleep(50 * std.time.ns_per_ms);return true;
  }
  fn agenteVerificarEstoque(ctx: *ContextoRuntime, ev: *EventoTransacional) bool {
    ctx.log("   [AgenteEstoque] Stock verificado no armazém central para #{d}", .{ev.id});std.time.sleep(30 * std.time.ns_per_ms);return true;}fn agenteDebitarConta(ctx: *ContextoRuntime, ev: *EventoTransacional) bool {ctx.log("   [AgenteTesouraria] A tentar debitar {d:.2} EUR da conta bancária...", .{ev.valor_eur});std.time.sleep(60 * std.time.ns_per_ms);// Simula sucesso no débito
ev.status = "DEBITADO";
return true;
}

fn agenteEstornarConta(ctx: *ContextoRuntime, ev: *EventoTransacional) bool {
  ctx.log("   [AgenteEstorno] SAGA ROLLBACK: Devolvendo fundos à conta e cancelando ordem #{d}.", .{ev.id});std.time.sleep(50 * std.time.ns_per_ms);ev.status = "ESTORNADO";return true;}fn agenteEmitirNota(ctx: *ContextoRuntime, ev: *EventoTransacional) bool {ctx.log("   [AgenteFaturacao] Nota fiscal emitida com sucesso para o evento #{d}.", .{ev.id});std.time.sleep(40 * std.time.ns_per_ms);ev.status = "CONCLUIDO";return true;}// ============================================================================// 6. MAIN - COMPILAÇÃO DA DSL 2FLOW E EXECUÇÃO// ============================================================================pub fn main() !void {var gpa = std.heap.GeneralPurposeAllocator(.{}){};defer _ = gpa.deinit();const allocator = gpa.allocator();
